...
*	getIconClass
*
*		Returns the icon class name for a given file extension. If the file is
*		a directory the class name extension is uppercase 'DIR' to distinguish
*		between the file extension 'dir'.
*
*	@param	pcFilename		Address C-string containing a filename
*	@param	bDirectory		Boolean indicating if this file is a directory.
*
*	@return		Address C-string containing the icon class name
**/
char *getIconClass( char *pcFilename, bool bDirectory )
{
	static char	cIconClass[MAX_PATH_SIZE];
	char	cExtension[MAX_PATH_SIZE],
			*pcExt;
	
	strcpy( cIconClass, "fileIcon" );
	if(!bDirectory)
	{
		if( (pcExt = strrchr(pcFilename, '.')) )
		{
			strncpyz( cExtension, ++pcExt, sizeof(cExtension)-1);
			strcat( cIconClass, (*cExtension ? strcap( cExtension ) : "Unknown") );
		}
		else
		{
			strcat( cIconClass, "Unknown");
		}
	}
	else
	{
		strcat( cIconClass, "DIR" );
	}
	// Append a generic icon classname in case the css classname for the file extension
	// is not defined.
	strcat( cIconClass, " fileIcon" );
	return cIconClass;
}
