define(["dojo/_base/declare",
			 ...
			 ], function (declare, lang, Deferred, json, request, Stateful, QueryResults,
										 QueryEngine, when, Mutex) {
	"use strict";
	...
	var FileObjectStore = declare([Stateful], {
		...
		constructor: function (/*Object*/ kwArgs) {
			...
			this._loadFinished   = new Deferred(); // Indicates if the initial load request has completed.
		...
		// defaultProperties: Object
		//		A JavaScript key:values pairs object whose properties are added to any new
		//		store object.
		defaultProperties: {},
		...
		moduleName: "cbTree/store/FileObjectStore",
		...
		_basePathSetter: function (/*String*/ value) {
			...
			if (typeof value === "string") {
				...
				throw new Error(this.moduleName + "::_basePathSetter(): basePath property must be a string");
			}
		},
			...
			if (value && lang.isObject(value)) {
				for (var prop in value) {
					if (this._reservedProp.indexOf(prop) != -1) {
						throw new Error(this.moduleName + "::_defaultPropertiesSetter(): No default allowed for property '"
																						 + prop + "'" );
					}
				}
			} else {
				throw new Error(this.moduleName + "::_defaultPropertiesSetter(): defaultProperties must be an object");
			}
		},
			...
			value.forEach( function (sortObj) {
				if (lang.isObject(sortObj)) {
					if (!("attribute" in sortObj)) {
						throw new Error(this.moduleName + "::_sortSetter(): 'attribute' property missing in sort object");
					}
				} else {
					throw new Error(this.moduleName + "::_sortSetter(): sort property must be an Array of objects");
				}
			});
			this.sort = value;
			return value;
		...
		_optionsSetter: function (/*String[] | String*/ value) {
			...
			if (!(value instanceof Array)) {
				if (typeof value !== "string") {
					throw new Error(this.moduleName + "::_optionSetter(): Options must be a keyword string"
		...
		_deleteFromStore: function (/*Object*/ dataObject) {
			...
			if (items && items.length) {
				return this._mutex.aquire( function () {
					...
					//  Purge sparse array after which we re-index..
		...
		_fetchFromServer: function (/*String*/ path, /*Object?*/ queryOptions, /*Deferred?*/ deferred) {
			...
			promise = this._xhrRequest( "GET", this.url, options);
			promise.then(
				function (dataObject) {
		...
		_setData: function (/*Object[]*/ data) {
			...
			for(var i = 0, l = data.length; i < l; i++) {
				this._primIndex[data[i][C_PATH_ATTR]] = i;
			}
		},
		...
		_updateFileStore: function (/*Object*/ dataObject) {
			...
			function updateFile(/*Object*/ item ) {
				// summary:
				...
				if (self._storeLoaded) {
					// Create a temporary place holder to prevent an evented put() from
					// trying to get the same item from the server before the new item
					// is actually stored. An evented put() tries to get the item first
					// to determine if it is a new or updated item. (See Evented.js).
		...
		//=========================================================================
		// Public dojo/store API methods

		add: function (object, options) {
			// summary:
			//		Not allowed.
			throw new Error(this.moduleName + "::add() not allowed, use get() and put() instead");
		},
		...
		mayHaveChildren: function (/*Object*/ object) {
			// summary:
			//		Return true if the object protentially has children otherwise false.
			// object:
			//		Store object.
			// tag:
			//		Public, Extension
			var file = this._fetchFromStore(this.getIdentity(object));
		...
		loadStore: function (/*Object*/ options) {
			// summary:
			//		Initiate store load.
			// options:
			//		A JavaScript key:value pairs object
			...
			var result       = this._loadFinished;
			var self         = this;

			if (!this._storeLoaded && !this._loadInProgress) {
				this._loadInProgress = true;
				result = this._fetchFromServer( null, queryOptions, this._loadFinished)
		...
		query: function (/*Object*/ query, /*Object?*/ options, /*Object[]?*/ _dataSet) {
			...
			if (!this._storeLoaded) {
				...
				return when (this._loadFinished, function () {
		...
		remove: function (/*String|number*/ id, /*Boolean?*/ _storeOnly) {
			...
			this.get(id).then(
				function (item) {
					if (!_storeOnly) {
						var result = self._xhrRequest( "DELETE", self.url, {path: item.path});
					} else {
		...
		rename: function (/*Object*/ object, /*String*/ newPath) {
			...
			if (newPath && typeof newPath === "string") {
				...
				this.get(itemId).then(
					function (item) {
						var result = self._xhrRequest("POST", self.url, {path: item.path, newValue: newPath});
						result.then(
							function (data) {
